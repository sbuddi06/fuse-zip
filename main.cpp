...
#define FUSEZIP_OPT(t, p, v) { t, offsetof(struct fusezip_param, p), v }
...
int main(int argc, char *argv[]) {
    ...
    if (!param.version) {
        ...
        if ((data = initFuseZip(PROGRAM, param.fileName, param.readonly, param.force_precise_time, param.passwd))
                == NULL) {
            fuse_opt_free_args(&args);
            return EXIT_FAILURE;
        }
